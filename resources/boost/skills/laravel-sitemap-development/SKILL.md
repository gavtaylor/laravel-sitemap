...
- **Custom HTML**: publish the view (`--tag=sitemap-views`) and edit `resources/views/vendor/sitemap/html.blade.php` directly to extend the app's own layout. It receives `$groups` (a `Collection<string, Collection<int, SitemapUrl>>` keyed by a headlined URL group, alphabetically sorted by label). Never invent new variables here - treat that as a fixed contract. Use `$url->label` for link text, never `$url->url` - a raw URL is a poor link label.
