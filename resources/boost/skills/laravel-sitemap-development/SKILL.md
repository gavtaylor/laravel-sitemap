...
- **Acronyms in labels/groups**: every label and group heading runs through `Str::headline()`, which title-cases each word with no idea an acronym should stay fully uppercase (`eca-committee` -> "Eca Committee"). Add the correction once to `label_glossary` (e.g. `'eca' => 'ECA'`) rather than overriding the full label of every affected route by hand - it's applied word-by-word, case-insensitively, to every label and group it appears in.
