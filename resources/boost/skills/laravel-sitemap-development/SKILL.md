...
- **Acronyms in labels/groups**: every label and group heading runs through `Str::headline()`, which title-cases each word with no idea an acronym should stay fully uppercase (`eca-committee` -> "Eca Committee"). Add the correction once to `label_glossary` rather than overriding the full label of every affected route by hand, matched case-insensitively. A single-word key (`'eca' => 'ECA'`) applies everywhere that word appears; a space-separated phrase key (`'reach newsletter' => 'REACH Newsletter'`) applies only to that exact run of words, for a word that would be wrong to correct everywhere (an unrelated route's ordinary use of "reach", say) - phrases match longest-first.
